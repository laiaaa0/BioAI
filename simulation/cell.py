...
class Cell():
    ...
    # Remember that the coordinate system is ...#TODO [from top-left? which
    # way is +ve x and +ve y?]
    ...
    def update(self, fire_list_current, fire_list_next,
               fire_grid, world_dims, offset_spread_modifiers):
        if not self.burn():     # Burning reduces remaining fuel.
            ...
            # This should perhaps be passed back and taken care of by the
            # calling code.
        ...
        for neighbour_coord, offset in self.get_neighbours(world_dims):
            ...
            if neighbour.get_state() == CellState.BURNABLE:
                # Stochasticity
                if (RANDOM_WEIGHTING * random.random() + (1 - RANDOM_WEIGHTING) *
                        offset_spread_modifiers[offset] >= (1 - FIRE_TRANSMISSION_PROBABILITY)):
                    # TODO Improvement: don't like modifying these from inside this function.
                    # These should perhaps be passed back and taken care of by
                    # the calling code.
    ...
    def get_neighbours(self, world_dims):
        ...
        for (i, j) in neighbourhood:
            (neighbour_x, neighbour_y) = (
                self.__coords[0] + i, self.__coords[1] + j)
            if (0 <= neighbour_x < world_dims[0]) and (
                    0 <= neighbour_y < world_dims[1]):
    ...
    def burn(self):
        # E.g. burn rate = 10 (%) => tree burns out in 10 iterations.
        self.__fuel = max(0, self.__fuel - CELL_BURN_RATE)
